...
class SVG(validation: Boolean = false, hasAttributes: HasAttributes = HasAttributesImpl(validation)) :
    ...
    /**
     * The x coordinate of the SVGs top left corner.
     */
    var x: String? by AttributeProperty(type = AttributeType.LengthOrPercentage)

    /**
     * The y coordinate of the SVGs top left corner.
     */
    ...
    /**
     * Create a svg element in this svg.
     */
    fun svg(block: SVG.() -> Unit): SVG = add(SVG(validation), block)

    override fun getAttributes(renderMode: RenderMode, isChild: Boolean): Map<String, String?> = if (renderMode == RenderMode.FILE) {
        mutableMapOf<String, String?>().also {
            if (!isChild) {
                it["xmlns"] = "http://www.w3.org/2000/svg"
            }
            it.putAll(attributes)
        }
    } else {
        super.getAttributes(renderMode, isChild)
    }

    override fun render(appendable: Appendable, renderMode: RenderMode, isChild: Boolean) {
        if (renderMode == RenderMode.FILE && !isChild) {
            appendable.append("<?xml version=\"1.0\" encoding=\"UTF-8\" ?>\n")
        }
        super.render(appendable, renderMode, isChild)
    }

    fun render(appendable: Appendable, renderMode: RenderMode) {
        render(appendable, renderMode, false)
    }
}
