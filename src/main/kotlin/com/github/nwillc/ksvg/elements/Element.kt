...
abstract class Element(
     ...
     */
    open fun render(appendable: Appendable, renderMode: RenderMode, isChild: Boolean = false) {
        appendable.append("<$name")
        getAttributes(renderMode, isChild)
            .entries
            .sortedBy { it.key }
            .forEach { entry ->
            appendable.append(' ')
        ...
        if (!hasContent()) {
            ...
            children.forEach {
                it.render(appendable, renderMode, isChild = true)
            }
            appendable.append("</$name>\n")
        }
    }
